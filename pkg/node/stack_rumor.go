...
type RumorStack struct {
	Messages map[string][]monguer.RumorMessage
	mux      sync.Mutex
}
...
func (stack *RumorStack) GetRumorMessage(origin string, id uint32) *monguer.RumorMessage {
	stack.mux.Lock()
	defer stack.mux.Unlock()
	messages, ok := stack.Messages[origin]
	if !ok || len(messages) <= 0 {
...
//AddMessage func
func (stack *RumorStack) AddMessage(msg monguer.RumorMessage) {
	stack.mux.Lock()
	defer stack.mux.Unlock()
	id := msg.ID
	messages, ok := stack.Messages[msg.Origin]
	if !ok {
		stack.Messages[msg.Origin] = []monguer.RumorMessage{msg}
	} else {
		lastMessageID := messages[len(messages)-1].ID
...
func (stack *RumorStack) GetLatestMessages() *[]monguer.RumorMessage {
	stack.mux.Lock()
	defer stack.mux.Unlock()
	var latestMessages = []monguer.RumorMessage{}
	for address := range stack.Messages {
...
func (stack *RumorStack) getStatusMessage() *monguer.StatusPacket {
	stack.mux.Lock()
	defer stack.mux.Unlock()
	var vector []monguer.PeerStatus
	for address := range stack.Messages {
		messages := stack.Messages[address]
		peerStatus := monguer.PeerStatus{Identifier: address, NextID: uint32(messages[len(messages)-1].ID + 1)}
		vector = append(vector, peerStatus)
	}
	return monguer.NewStatusPacket(&vector, "")
}

func (stack *RumorStack) getRumorStack() *map[string][]monguer.RumorMessage {
	stack.mux.Lock()
	defer stack.mux.Unlock()
	return &stack.Messages
}

func (stack *RumorStack) getFirstMissingMessage(comparedMessages *[]monguer.PeerStatus) *monguer.RumorMessage {
